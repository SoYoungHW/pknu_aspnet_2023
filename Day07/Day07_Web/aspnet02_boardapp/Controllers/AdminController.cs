    ...
    public class AdminController : Controller
    {
        ...
        public async Task<IActionResult> CreateRole(CreateRoleModel model)
        {
            if (ModelState.IsValid) 
            {
                ...
                var result = await _roleManager.CreateAsync(role); // 지정한 권한명이 DB(aspnetroles)에 들어감
        ...
        {
            var role = await _roleManager.FindByIdAsync(id); // aspnetroles 테이블에서 Id로 조회(SELECT)
        ...
        {
            var role = await _roleManager.FindByIdAsync(model.Id); // aspnetroles 테이블에서 Id로 조회(SELECT)
        ...
        public async Task<IActionResult> EditUsersInRole(List<UserRoleModel> model, string roleId)
        {
            ...
            foreach (var user in model)
            {
                ...
                {
                    result = await _userManager.AddToRoleAsync(userObj, role.Name); // 사용자에게 권한을 할당
                }
